...
router.post('/scan', requireAuth, async (req, res) => {
  try {
    const { plate, vehicleId, driverId, descriptor, consent } = req.body;
    ...
    if (
      ...
      return res.status(400).json({
        error: 'A plate, selected driver, selected vehicle, and valid face descriptor are required.',
      });
    }
    if (!consent) {
    ...
    res.status(err.statusCode || 500).json({
      error: err.statusCode ? err.message : 'Something went wrong processing the scan. Please try again.',
    });
  }
});
